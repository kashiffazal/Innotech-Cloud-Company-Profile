...
export default function Slide14Testimonials({ theme = 'light' }) {
  ...
  const testimonials = [
    {
      quote: "Innotech Cloud transformed our candidate recruitment process with custom AI screening features. Engineering quality, delivery speed, and communication were world-class.",
      author: "David Miller",
    ...
    {
      quote: "Our transition to Block IMS ERP was completely seamless. They handled legacy data migration effortlessly and engineered a system that handles 14 enterprise warehouse locations.",
    ...
    {
      quote: "The payment processing architecture built by Innotech Cloud reduced checkout failure rates by 99.4%. Outstanding technical partnership and ongoing SLA support.",
      author: "Marcus Vance",
    ...
    {
      quote: "Webo 360 has grown 3x faster since partnering with Innotech Cloud. Their team built a scalable multi-tenant architecture that handles millions of automated marketing tasks daily.",
  ...
  return (
    <div className={`slide-container relative w-full h-full overflow-hidden flex flex-col justify-between p-6 sm:p-8 md:p-10 transition-colors duration-300 ${
      ...
      <div className={`relative z-10 shrink-0 flex items-center justify-between border-b pb-3 ${
        ...
        <div>
          <span className="text-xs font-bold tracking-widest text-cyan-500 uppercase">Client Endorsements</span>
          <h2 className="font-black italic uppercase tracking-tight text-2xl sm:text-3xl mt-0.5" style={{ color: isDark ? "#E0F2FE" : "#1E3A8A" }}>
            CLIENT <span style={{ color: isDark ? "#38BDF8" : "#0891B2" }}>TESTIMONIALS</span>
          </h2>
        </div>
      ...
      <div className="relative z-10 flex-1 overflow-y-auto min-h-0 my-auto py-3 pr-1 grid grid-cols-1 md:grid-cols-2 gap-4 items-stretch">
        {testimonials.map((t, idx) => (
          <div 
            key={idx}
            className={`p-4 sm:p-5 rounded-2xl border transition-all flex flex-col justify-between group ${
              isDark 
          ...
          >
            <div>
              ...
              {/* Quote Body */}
              <p className={`text-xs sm:text-sm leading-relaxed italic mb-3 ${
            ...
            <div className="pt-2.5 border-t border-slate-200 dark:border-slate-800 flex items-center space-x-3">
              ...
              <div className="min-w-0 flex-1">
                <h4 className={`text-xs font-extrabold truncate ${isDark ? 'text-white' : 'text-slate-900'}`}>
                  {t.author}
                </h4>
                <p className={`text-[10px] truncate ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                  {t.role} &bull; <span className="font-semibold text-cyan-600 dark:text-cyan-400">{t.company}</span>
                </p>
              </div>
            </div>
          </div>
