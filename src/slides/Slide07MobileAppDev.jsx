...
import { Smartphone, Layers, Cpu, Database, Bell, Rocket, ShieldCheck, MapPin } from 'lucide-react';

export default function Slide07MobileAppDev({ theme = 'light' }) {
  const isDark = theme === 'dark';

  const capabilities = [
    { title: "Cross-Platform Efficiency", desc: "Flutter and React Native for unified iOS & Android development with 60FPS native speed.", icon: Layers },
    { title: "Native Hardware Access", desc: "Biometrics (FaceID/TouchID), Bluetooth BLE, camera, accelerometer, and sensor APIs.", icon: Cpu },
    { title: "Offline-First Data Sync", desc: "Local Encrypted DB (SQLite/Realm) with automatic background cloud synchronization.", icon: Database },
    { title: "Push Notification Engines", desc: "Targeted push messaging via Firebase Cloud Messaging (FCM) & Apple APNs.", icon: Bell },
    { title: "App Store Publishing & ASO", desc: "End-to-end Apple App Store & Google Play publishing, guidelines compliance, and ASO.", icon: Rocket },
    { title: "Real-Time Geo-Tracking", desc: "Background location tracking, interactive maps integration, and socket streams.", icon: MapPin }
  ];
      ...
      <div className={`relative z-10 shrink-0 flex items-center justify-between border-b pb-3 ${
        ...
        <div>
          ...
          <h2 className="font-black italic uppercase tracking-tight text-2xl sm:text-3xl mt-0.5" style={{ color: isDark ? "#E0F2FE" : "#1E3A8A" }}>
            MOBILE APP <span style={{ color: isDark ? "#38BDF8" : "#0891B2" }}>DEVELOPMENT</span>
          </h2>
        </div>
      ...
      <div className="relative z-10 flex-1 overflow-y-auto min-h-0 my-auto py-3 pr-1 grid grid-cols-1 lg:grid-cols-12 gap-5 items-stretch">
        ...
        <div className="lg:col-span-5 p-5 sm:p-6 rounded-2xl bg-gradient-to-br from-[#031945] to-[#0A3D91] text-white shadow-xl flex flex-col justify-between border border-cyan-500/30">
          <div>
            ...
            <h3 className="text-lg font-extrabold leading-snug mb-2 text-white">iOS & Android Mobile Solutions</h3>
            <p className="text-slate-200 text-xs sm:text-sm leading-relaxed">
              We turn mobile app ideas into elegant, high-performing, and secure iOS & Android applications tailored to modern user expectations.
            </p>
          </div>
