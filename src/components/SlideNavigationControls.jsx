...
export default function SlideNavigationControls({
  ...
  return (
    <footer className={`no-print h-14 border-t transition-colors duration-300 px-4 sm:px-6 flex items-center justify-between z-50 shrink-0 select-none ${
      isDark 
      ...
      <div className="flex flex-col items-center space-y-1 max-w-md w-full px-4">
        <div className="flex items-center space-x-2 text-xs font-semibold">
          <span className="text-[#0A3D91] dark:text-cyan-400 font-mono font-bold">
            {String(currentSlide + 1).padStart(2, '0')} / {String(totalSlides).padStart(2, '0')}
          </span>
          <span className="text-slate-300 dark:text-slate-700">&bull;</span>
          <span className="truncate max-w-[200px] sm:max-w-[280px] text-slate-700 dark:text-slate-300">
            {slideTitles[currentSlide] || `Slide ${currentSlide + 1}`}
          </span>
        </div>

        {/* Dots & Progress Line */}
        <div className="w-full max-w-xs flex items-center space-x-1 justify-center">
      ...
      <button
        ...
        className={`flex items-center space-x-1.5 px-4 py-1.5 rounded-lg text-xs font-bold transition-all shadow-sm ${
